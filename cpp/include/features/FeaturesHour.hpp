...
public:
  ...
  void compute_and_store() {
    if (!lob_feature_->depth_updated || !global_store_) return;
    
    const auto &depth_buffer = lob_feature_->depth_buffer;
    if (depth_buffer.size() < 2 * LOB_FEATURE_DEPTH_LEVELS) return;
    ...
    if (curr_time_idx != last_time_idx_) {
      if (tick_count_ > 0 && last_time_idx_ > 0) {
        ...
        data.support_level = (sum_weights_ > 0.0) ? 
                             static_cast<float>(sum_low_weighted_ / sum_weights_) : low_;
        
        // Resistance level: weighted average of high prices
        data.resistance_level = (sum_weights_ > 0.0) ? 
                                static_cast<float>(sum_high_weighted_ / sum_weights_) : high_;
