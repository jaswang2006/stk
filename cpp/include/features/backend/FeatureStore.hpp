...
/*
EVENT-DRIVEN FEATURE STORE ARCHITECTURE
========================================

CORE DESIGN:
- Event-driven processing with nested trigger cascade
- Columnar storage with parent pointer mechanism
- Zero-copy tensor export for ML pipelines
- Complete macro-based code generation

EXTENSIBILITY:
--------------
To add L4/L5/L6/L7 levels, user only needs to:
1. Define LEVEL_4_FIELDS(X), LEVEL_5_FIELDS(X), etc. in FeaturesConfig.hpp
2. Add entries to ALL_LEVELS(X) macro
3. Add corresponding trigger/updater functions in Features.hpp

ALL OTHER CODE AUTO-GENERATED BY MACROS:
- init_L*_columns functions
- push_L*_fields functions
- switch statements for level handling
- template specializations
- tensor export logic
- field counting and sizing

DESIGN TRADE-OFFS:
------------------
Performance vs Flexibility: Function pointers (fast) over std::function
Memory vs Access: Parent pointers (compact) over data duplication
Compile-time vs Runtime: Complete macro generation for zero runtime overhead
Type Safety vs Performance: Type erasure for heterogeneous storage

SCALABILITY:
------------
- Row insertion: O(1) amortized
- Level cascade: O(LEVEL_COUNT)
- Memory growth: O(total_rows * fields_per_level)
- Export: Full expansion O(total_features * max_rows)
- Supports: 1000+ features, 100M+ rows per level
*/
